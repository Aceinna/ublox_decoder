...
static int adjweek(rtcm_t *rtcm, double tow)
{
    double tow_p,diff_tow;
    int week;
    
    /* if no time, get cpu time */
    if (rtcm->time.time==0) rtcm->time=utc2gpst(timeget());
    tow_p=time2gpst(rtcm->time,&week);
	tow = fmod(tow, 7 * 24 * 3600);
	/* consider for the invalid time tag */
	if (tow == 0.0 && fabs((tow - tow_p) / (24.0 * 3600.0)) > 1.5) return -1;
...
static int decode_head1001(rtcm_t *rtcm, int *sync)
{
	...
	if (adjweek(rtcm, tow)<0) return -1;
